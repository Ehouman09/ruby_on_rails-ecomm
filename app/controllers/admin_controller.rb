class AdminController < ApplicationController
  ...
  def index
    ...
    @quick_stats = {
      ...
      avg_sale: Order.where(created_at: Time.now.midnight..Time.now).average(:total).present? ? 
      Order.where(created_at: Time.now.midnight..Time.now).average(:total).round() : 0,
      per_sale: OrderProduct.joins(:order).where(orders: {created_at: Time.now.midnight..Time.now}).average(:quantity).present? ? 
      OrderProduct.joins(:order).where(orders: {created_at: Time.now.midnight..Time.now}).average(:quantity).round() : 0
    }
